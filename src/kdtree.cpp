...
bool KdTree::GetClosestPoint(const PointType &pt, std::vector<int> &closest_idx, int k) {
    if (k > size_) {
        std::cerr << "cannot set k larger than cloud size: " << k << ", " << size_ << std::endl;
        return false;
    }
    k_ = k;
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [this, &cloud, &matches, &k](int idx) {
        ...
        for (int i = 0; i < k; ++i) {
            ...
            if (i < closest_idx.size()) {
                ...
                matches[idx * k + i].first = kINVALID_ID;
            }
        }
    });
...
void KdTree::ComputeDisForLeaf(const Eigen::Vector3f &pt, KdTreeNode *node,
...
bool KdTree::FindSplitAxisAndThresh(const std::vector<int> &point_idx, int &axis, float &th, std::vector<int> &left, std::vector<int> &right) {
    ...
    Eigen::Vector3f var;
    Eigen::Vector3f mean;
    utils::ComputeMeanAndCovDiag(point_idx, mean, var, [this](int idx) { return cloud_[idx]; });
    int max_i, max_j;
...
void KdTree::PrintAll() {
    for (const auto &np : nodes_) {
        ...
        if (node->left_ == nullptr && node->right_ == nullptr) {
            ...
            std::cout << "node: " << node->id_ << ", axis: " << node->axis_index_ 
                        << ", th: " << node->split_thresh_ << std::endl;
        }
    }
}
