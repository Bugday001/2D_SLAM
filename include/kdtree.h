...
class KdTree {
   ...
   private:
    ...
    inline Eigen::Vector3f ToVec3f(const PointType &pt) { return Eigen::Vector3f(pt(0), pt(1), 0.0); }
    // Knn 相关
    /**
     ...
     */
    void Knn(const Eigen::Vector3f& pt, KdTreeNode* node, std::priority_queue<NodeAndDistance>& result) const;
     ...
     * @param node  Kdtree 节点
     */
    void ComputeDisForLeaf(const Eigen::Vector3f& pt, KdTreeNode* node, std::priority_queue<NodeAndDistance>& result) const;
